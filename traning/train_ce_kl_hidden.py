...
class DistillationConfig:
    ...
    output_dir: str = "runs/qwen3b_smol360m_ce_kl_hidden_gpu1"
    save_steps: int = 500
...
class HiddenProjector(nn.Module):
    def __init__(self, student_hidden_size: int, teacher_hidden_size: int):
        super().__init__()
        self.projector = nn.Linear(student_hidden_size, teacher_hidden_size)
        nn.init.normal_(self.projector.weight, std=0.02)
...
class DistillationTrainer:
    def __init__(self, teacher, student, vocab_projector, hidden_projector, tokenizer, config, logger):
    ...
    def trainable_parameters(self) -> List[nn.Parameter]:
        ...
        params.extend(list(self.hidden_projector.parameters()))
        return params
    ...
    def compute_hidden_loss(
        ...
        projected_student_hidden = self.hidden_projector(student_hidden.to(dtype))
...
def run_training(config: DistillationConfig) -> None:
    ...
    teacher, student, vocab_projector, hidden_projector, tokenizer = load_models(config, logger)
