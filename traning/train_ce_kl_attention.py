...
class DistillationConfig:
    ...
    output_dir: str = "runs/qwen3b_smol360m_ce_kl_attention"
    save_steps: int = 500
...
class DistillationTrainer:
    ...
    def compute_attention_relation_loss(
        ...
        #Average the attention weights across all heads(This is a wrong steps that I choice, whcih result in model's poor performance) 
        student_relation = student_attention.mean(dim=1).float()
        teacher_relation = teacher_attention.mean(dim=1).float()

        seq_len = student_relation.size(-1)
        # mask future tokens 
        causal_mask = torch.tril(torch.ones(seq_len, seq_len, device=self.device))

        # padding mask
        pair_mask = (attention_mask.unsqueeze(1) * attention_mask.unsqueeze(2)).float()
        
        full_mask = pair_mask * causal_mask #Only compare the attention between real tokens.

        squared_error = (student_relation - teacher_relation) ** 2
        masked_error = squared_error * full_mask

        return masked_error.sum() / full_mask.sum().clamp_min(1.0)
    ...
    def compute_batch_loss(
        ...
        student_outputs, student_hidden, projected_logits = self.forward_student(
            input_ids=input_ids,
            attention_mask=attention_mask,
            output_attentions=True,
        )
        ...
        attn_loss = self.compute_attention_relation_loss(
            student_attention=student_outputs.attentions[-1],
