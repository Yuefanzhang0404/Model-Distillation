...
class DistillationConfig:
    ...
    output_dir: str = "runs/distill_run"
    save_steps: int = 500
...
class VocabProjector(LinearProjector):
    pass


class HiddenProjector(LinearProjector):
    pass
...
class DistillationDataset(Dataset):
    def __init__(
        ...
        self.texts = self._read_jsonl_texts(Path(data_path), data_size)

        if not self.texts:
            raise ValueError(f"No valid training samples found in {data_path}")

    def _read_jsonl_texts(self, path: Path, data_size: int) -> List[str]:
        ...
        for file_path in files:
            with file_path.open("r", encoding="utf-8") as f:
                for line in f:
                    text = self._parse_text(line)

                    if text:
                        texts.append(text)

                    if len(texts) >= data_size:
                        return texts

        return texts

    @staticmethod
    def _parse_text(line: str) -> Optional[str]:
        try:
...
def average_records(
    ...
    if include_step_end:
        prefix["global_step_end"] = global_step
    else:
...
class DistillationTrainer:
    def __init__(
        ...
        hidden_projector: Optional[HiddenProjector],
        tokenizer,
        ...
        self.hidden_projector = hidden_projector
        self.tokenizer = tokenizer
        self.config = config
        self.logger = logger
    ...
    def _trainable_modules(self) -> List[nn.Module]:
        modules: List[nn.Module] = [self.student, self.vocab_projector]

        if self.hidden_projector is not None:
            modules.append(self.hidden_projector)
    ...
    def compute_ce_loss(
        ...
        student_logits = student_logits[..., :-1, :].contiguous().float()
    ...
    def compute_kd_loss(
        ...
        student_logits = student_logits[..., :-1, :].contiguous().float()
        teacher_logits = teacher_logits[..., :-1, :].contiguous().float()
        labels = labels[..., 1:].contiguous()
        ...
        valid_tokens = labels != self.ignore_index
    ...
    def compute_hidden_loss(
        ...
        if self.hidden_projector is None:
            return torch.tensor(0.0, device=self.device)

        dtype = self.hidden_projector.projector.weight.dtype

        student_hidden = student_hidden.to(dtype)
        teacher_hidden = teacher_hidden.to(dtype)

        projected_student_hidden = self.hidden_projector(student_hidden)
        mask = attention_mask.unsqueeze(-1).to(dtype)

        squared_error = (projected_student_hidden - teacher_hidden) ** 2
        masked_error = squared_error * mask

        numerator = masked_error.float().sum()
    ...
    def combine_losses(
        ...
        if self.config.mode == "ce_kl":
            return {
                "total_loss": self.config.alpha * ce_loss + self.config.beta * kd_loss,
                "ce_loss": ce_loss,
                "kd_loss": kd_loss,
                "hidden_loss": zero,
            }

        if self.config.mode == "ce_kl_hidden":
            return {
                "total_loss": (
                    self.config.alpha * ce_loss
                    + self.config.beta * kd_loss
                    + self.config.gamma * hidden_loss
                ),
                "ce_loss": ce_loss,
                "kd_loss": kd_loss,
    ...
    def _compute_batch_losses(
        ...
        need_teacher_hidden = self.config.mode == "ce_kl_hidden"

        teacher_outputs = self._forward_teacher(
            input_ids=input_ids,
            attention_mask=attention_mask,
            output_hidden_states=need_teacher_hidden,
        )

        _, student_hidden, projected_logits = self._forward_student(
            input_ids=input_ids,
            attention_mask=attention_mask,
        )

        ce_loss = self.compute_ce_loss(projected_logits, labels)
    ...
    def _record_step(
        ...
        return {
            "total_loss": self._to_float(losses["total_loss"]),
            "ce_loss": self._to_float(losses["ce_loss"]),
            "kd_loss": self._to_float(losses["kd_loss"]),
            "hidden_loss": self._to_float(losses["hidden_loss"]),
    ...
    def _maybe_log(self, epoch: int, records: List[Dict[str, float]]) -> None:
        ...
        self.logger.info(
            "Step %s | Loss: %.4f | CE: %.4f | KD: %.4f | Hidden: %.4f | Tok/s: %.2f",
            self.global_step,
    ...
    def _maybe_save_checkpoint(self, epoch: int, records: List[Dict[str, float]]) -> None:
        ...
        self._save_checkpoint(step=self.global_step, metrics=metrics)

        if metrics["avg_total_loss"] < self.best_loss:
            self.best_loss = metrics["avg_total_loss"]
            self._save_checkpoint(step=self.global_step, metrics=metrics, tag="best")

    def _cleanup_old_checkpoints(self, keep_last: int = 2) -> None:
    ...
    def _save_checkpoint(
        ...
        if self.hidden_projector is not None:
            torch.save(
                self.hidden_projector.state_dict(),
                checkpoint_dir / "hidden_projector.pt",
            )
    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int) -> Dict[str, Any]:
        ...
        for step, batch in enumerate(dataloader):
            record = self._train_step(batch)
            epoch_records.append(record)

            if (step + 1) % self.config.gradient_accumulation_steps == 0:
                self._optimizer_step()
                self._maybe_log(epoch, epoch_records)
                self._maybe_save_checkpoint(epoch, epoch_records)
        ...
        epoch_summary = average_records(
            ...
            global_step=self.global_step,
            mode=self.config.mode,
            include_step_end=True,
        )

        append_jsonl(self.epoch_metrics_path, epoch_summary)

        self.logger.info(
            "Epoch %s finished | Total: %.4f | CE: %.4f | KD: %.4f | Hidden: %.4f | Tok/s: %.2f",
            epoch + 1,
    ...
    def _train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        normalized_loss = losses["total_loss"] / self.config.gradient_accumulation_steps
    ...
    def train(self, dataloader: DataLoader) -> None:
        ...
        for epoch in range(self.config.num_train_epochs):
            epoch_summary = self.train_epoch(dataloader, epoch)
            epoch_summaries.append(epoch_summary)

        final_metrics = epoch_summaries[-1] if epoch_summaries else {"mode": self.config.mode}
        self._save_checkpoint(self.global_step, final_metrics, tag="final")
...
def build_projectors(
    ...
    if config.mode == "ce_kl_hidden":
        hidden_projector = HiddenProjector(
            student.config.hidden_size,
            teacher.config.hidden_size,
        ).to(device=device, dtype=amp_dtype)

    return vocab_projector, hidden_projector
...
def load_projector_checkpoint(
    ...
    if hidden_projector is None:
        return

    hidden_projector_path = checkpoint_dir / "hidden_projector.pt"

    if hidden_projector_path.exists():
        logger.info("Loading hidden projector from: %s", hidden_projector_path)
        hidden_projector.load_state_dict(
            torch.load(hidden_projector_path, map_location="cpu")
        )
...
def log_model_info(
    ...
    if hidden_projector is not None:
        logger.info(
            "Trainable hidden projector params: %s",
            f"{count_trainable_parameters(hidden_projector):,}",
        )
...
def load_models(config: DistillationConfig, logger: logging.Logger):
    ...
    vocab_projector, hidden_projector = build_projectors(
        config=config,
        student=student,
        teacher=teacher,
        amp_dtype=amp_dtype,
    ...
    return teacher, student, vocab_projector, hidden_projector, tokenizer
...
def run_training(config: DistillationConfig) -> None:
    ...
    teacher, student, vocab_projector, hidden_projector, tokenizer = load_models(
        config,
        logger,
    )
...
if __name__ == "__main__":
    cfg = DistillationConfig(
        mode="ce_kl",
        output_dir="runs/qwen3b_smol360m_ce_kl",
        device="cuda:0",
        alpha=0.5,
        beta=0.5,
    )
